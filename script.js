...
label.innerHTML =
  "<i>Write your name to find in which country your name is common</i> <br>";
      ...
      for (let i = 0; i < 2; i++) {
        ...
        try {
          async function Cname() {
            var ddata = await fetch(
              "https://restcountries.eu/rest/v2/alpha/" +
              resp.country[i].country_id
            );
            var rres = await ddata.json();
            console.log(rres.name);
            p.innerText =
              i +
              1 +
              " Country you may belong to is:  " +
              rres.name +
              " with probability :" +
              resp.country[i].probability;
